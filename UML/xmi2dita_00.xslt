...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:xmi="http://www.omg.org/spec/XMI/20131001" exclude-result-prefixes="#all">
	<xsl:output method="xml" doctype-public="-//OASIS//DTD DITA Reference//EN"
		doctype-system="technicalContent/dtd/reference.dtd" encoding="UTF-8" byte-order-mark="no"
...
-->
	...
	<xsl:template name="TopLoop">
		...
		<xsl:for-each
			select="$rootElt//xmi:XMI/xmi:Extension/elements/element[@xmi:type = 'uml:Class']">
		...
		<xsl:for-each
			select="$rootElt//xmi:XMI/xmi:Extension/elements/element[@xmi:type = 'uml:DataType']">
			<xsl:variable name="secName" select="@name"/>
			<xsl:call-template name="elementLoop">
				<xsl:with-param name="secName" select="$secName"/>
			</xsl:call-template>
		...
		<xsl:element name="section">
				...
				<xsl:choose>
					<xsl:when test="$docY != ''">
						<xsl:variable name="doc0"
							select="fn:replace($docX, '&lt;ul&gt;', '&lt;/p&gt;&lt;ul&gt;')"/>
						<xsl:variable name="doc1"
							select="fn:replace($doc0, '&lt;/ul&gt;', '&lt;/ul&gt;&lt;p&gt;')"/>
						<xsl:variable name="doc2"
							select="fn:replace($doc1, '&lt;ol&gt;', '&lt;/p&gt;&lt;ol&gt;')"/>
						<xsl:variable name="doc3"
							select="fn:replace($doc2, '&lt;/ol&gt;', '&lt;/ol&gt;&lt;p&gt;')"/>
						<p>
							<xsl:value-of select="$doc3" disable-output-escaping="yes"/>
						</p>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:variable name="array" as="element()*">
				<xsl:for-each select="links/Generalization">
					...
					<xsl:variable name="connectorName"
						select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/target/model/@name"/>
					<xsl:choose>
						<xsl:when test="$connectorName != $secName">
							<Item>
								<xsl:value-of select="fn:number(1)"/>
							</Item>
						</xsl:when>
						<xsl:otherwise>
							<Item>
								<xsl:value-of select="fn:number(0)"/>
							</Item>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
						...
						<tbody>
							<xsl:for-each select="attributes/attribute">
								<xsl:variable name="doc"
									select="fn:tokenize(documentation/@value, '&#xA;')"/>
								<row rowsep="1">
									<entry colname="1">
										<xsl:value-of
											select="replace(@name, ':', fn:concat(':', $zeroWidthSpace))"
										/>
									</entry>
									<entry colname="2">
										<xsl:value-of
											select="replace(properties/@type, ':', fn:concat(':', $zeroWidthSpace))"
										/>
									</entry>
									<entry colname="3">
										<xsl:value-of
											select="concat(bounds/@lower, '..', bounds/@upper)"/>
									</entry>
									<entry colname="4">
							...
							-->
								<xsl:if
									test="not(preceding::Association[@xmi:id = current()/@xmi:id] and ./@start = ./@end)">
	...
	<xsl:template name="connectorLoop">
		<xsl:param name="secName"/>
		<xsl:variable name="connectorId" select="@xmi:id"/>
		<xsl:variable name="modelNameTarget"
			select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/target/model/@name"/>
		<xsl:variable name="modelNameSource"
			select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/source/model/@name"/>
		<xsl:if test="$modelNameTarget != $secName or $modelNameSource = $modelNameTarget">
			<xsl:variable name="documentation"
				select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/documentation/@value"/>
			<xsl:variable name="docX">
				<xsl:call-template name="FormatString">
					<xsl:with-param name="docAttr1" select="$documentation"/>
				</xsl:call-template>
			...
			<row rowsep="1">
				<entry colname="1">
					<xsl:value-of
						select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/labels/@mt"
					/>
				</entry>
				<entry colname="2">
					<xsl:value-of
						select="replace($modelNameTarget, ':', fn:concat(':', $zeroWidthSpace))"/>
				</entry>
				<entry colname="3">
					<xsl:value-of
						select="/xmi:XMI/xmi:Extension/connectors/connector[@xmi:idref = $connectorId]/target/type/@multiplicity"
					/>
				</entry>
				<entry colname="4">
